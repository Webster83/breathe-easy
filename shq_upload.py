'''
shq_upload.py is a Python script designed to facilitate the upload of CPAP data files to SleepHQ.
It handles authentication, file discovery, uploading, and processing of the data through
SleepHQ's API. 

The script can be run from the command line with appropriate arguments or called from another
Python script as a wrapper function.

This script is desgined to be platform agnostic and should work on Windows, Mac, and Linux.
It also includes logic to inject the system truststore into the SSL context on Windows
to avoid SSL errors for users in corporate environments with custom CAs. While only
tested against Resmed CPAP data, it should work with any CPAP data that is imported
from the sd card.
...
# On Windows, inject the system truststore into the SSL context, this needs to be done before
# importing requests. This will allow for any corporate or custom CAs to be recognized by
# requests module
...
if sys.platform.startswith("win"):
    try:
        ...
        print(f"""Could not inject truststore into SSL context, {e}. You will experience SSL
              errors if your system uses custom or corporate CAs. Exit by typing Ctrl-C,
              or proceed to try.""")



def get_shq_access_token(client_id,client_secret,verbose)-> str:
    '''Obtain an access token from SleepHQ using the provided client_id and client_secret.
    Returns a string in the format 'Bearer <access_token>' if successful, or exits the program if
    there is an error.
...
def get_shq_team_id(token) -> str:
    ...
    '''
    get_shq_team_id retrieves the current team ID associated with the authenticated user
    from SleepHQ using the provided access token.
...
def get_files(dir_path)-> list[dict]:
    ...
    """
    Walks through the given directory and its subdirectories to find all files to upload,
    that do not start with a dot.
    For each file, it computes the absolute directory path, filename, SleepHQ-style relative path,
    and content hash.
...
def get_shq_import_req_id(team_id, token) -> str:

    """
    Ges the SleepHQ Import Request ID
    
    :param str id: Sleep HQ Team ID
    :param str token: Sleep HQ Bearer Token

    :return: Sleep HQ Import Request ID
    :rtype: str
...
def process_shq_imports(import_id, token)-> None:

    '''
    process_shq_imports sends a request to SleepHQ to start processing the uploaded files for a 
    given import reservation ID.
...
def validate_import_to_shq(import_id, token)->None:

    '''
    validate_import_to_shq checks the status of the import processing for a given import
    reservation ID and prints the result.
...
def run_upload(clt_id:str, clt_sec:str, dpath:str, verbose:bool) -> None:

    '''
    run_upload orchestrates the entire upload process to SleepHQ, including authentication,
    file discovery, uploading, processing, and validation.
